...
В качестве параметра в приложение подается адрес мультикаст-группы, который может быть как IPv4, так и IPv6. Если параметра нет, то за адрес группы берется 224.1.1.1
